function detect_points(img::AbstractArray, quality::Number, min_dist::Int )
    ...
    layered_responses = cat(responses...; dims = 3); # Layer All Together

    out = pick_maximum_response(layered_responses)   # Take best point for each pixel
...
function detect_points(img::OffsetArray, quality::Number, min_dist::Int)
    out = detect_points(parent(img), quality, min_dist)
    return OffsetArray(out, axes(img))
end
